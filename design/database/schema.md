...
erDiagram
    ...
    BOX {
        text id PK "uuid"
        text box_type "Message|Goal|Memory|Decision|Secret|Challenge|Letter"
        text title "nullable"
    ...
    BOX_TYPE_TEMPLATE {
        text box_type PK "Message|Goal|Memory|Decision|Secret|Challenge|Letter"
        text display_name
        text icon_name
        text color_hex
...
| `box_type` | TEXT | NOT NULL, CHECK in ('Message','Goal','Memory','Decision','Secret','Challenge','Letter') | Loại hộp (F-01/F-37) |
...
| Secret | Bí mật | eye-off | accent | "Bí mật này bây giờ còn quan trọng không?" | "Có một điều mình chưa muốn nói ra..." |
| Challenge | Thử thách | trophy | accent | "Bạn đã hoàn thành thử thách chưa?" | "Thử thách mình muốn hoàn thành là..." |
| Letter | Lá thư | mail | accent | "Bạn cảm thấy thế nào khi đọc lại lá thư này?" | "Gửi tôi của tương lai..." |
...
export async function migrateDbIfNeeded(db: SQLiteDatabase) {
  ...
  // v3->v4 adds box_prediction.
  // v4->v5 rebuilds reflection_question for post-open reflection.
  // v5->v6 rebuilds box to widen box_type CHECK to include Secret, Challenge, Letter.
...
3. **Validation tạo hộp (enforce ở logic):**
   ...
   - `box_type` thuộc 7 giá trị hợp lệ: `Message`, `Goal`, `Memory`, `Decision`, `Secret`, `Challenge`, `Letter`.
...
- `DATABASE_VERSION = 6`.
- Append only `if (currentVersion === 5)` in `src/db/database.ts`.
- Rebuild `box` safely to widen only the `box_type` CHECK constraint to include `Secret`, `Challenge`, and `Letter`.
- Copy every existing `box` column and row, including `is_deleted`; do not add tables or columns.
- Keep `PRAGMA foreign_keys = OFF` during `DROP`/`RENAME`, then recreate `idx_box_unlock_date`, `idx_box_is_opened`, `idx_box_type`, and `idx_box_list` before turning foreign keys back on.
- Existing child rows in `reflection_question`, `notification_schedule`, `box_teaser`, and `box_prediction` remain tied to `box(id)` after the rename.
