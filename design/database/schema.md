...
### 4.4. Bảng `box_type_template` (TÙY CHỌN - khuyến nghị hardcode)
...
### 4.5. Hard delete vs Soft delete

- **MVP khuyến nghị HARD DELETE** (xóa thật bản ghi box, CASCADE xóa question & notification_schedule, đồng thời xóa file ảnh trên file system + hủy notification). Đơn giản, đúng kỳ vọng người dùng "xóa hộp".
...
3. **Validation tạo hộp (enforce ở logic):**
   ...
   - `unlock_date ≥ today + 1 tháng` (Q3, AC-02). Preset: +1 tháng, +3 tháng, +6 tháng, +1 năm.
   - `box_type` thuộc 4 giá trị hợp lệ.
4. **Tạo hộp là 1 transaction** (NFR-R1): INSERT `box` + (nếu có) `reflection_question` + `notification_schedule` trong `db.withTransactionAsync`. Việc gọi `scheduleNotificationAsync` (side-effect ngoài DB) thực hiện **trước** rồi lưu `notification_identifier`; nếu DB transaction fail thì hủy notification để tránh rác.
...
6. **Xóa hộp**: hard delete box (CASCADE xóa question + notification_schedule) **và** (a) hủy notification qua `cancelScheduledNotificationAsync(notification_identifier)` nếu còn, (b) xóa file ảnh `image_path` qua expo-file-system.
